...
| 2026-05-17 | Added Continuous Monitoring Dashboard for Automation Project Overview. Dashboard appears when Dashboard output is selected AND run type is Recurring. Shows: monitoring status (active/paused/not scheduled), run health cards, exception health cards, case health cards, workflow health table, quick navigation actions. Empty/partial states: prompts for selecting workflows, running initial baseline, activating schedule. Non-recurring or non-dashboard projects show standard overview with informational hints. Dashboard tooltip added to wizard Output Types checkbox. Compliance/IA overviews unchanged. |

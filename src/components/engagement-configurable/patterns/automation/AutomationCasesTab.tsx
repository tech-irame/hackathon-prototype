...
import {
  ...
  type AutomationCasesState, type AutomationCase, type CaseStatus, type CasePriority, type CaseCategory,
...
export default function AutomationCasesTab({ engagement, runsState, casesState, onUpdateCases, onNavigateTab }: Props) {
  ...
  const createCaseFromCandidate = (exId: string) => {
    ...
    const c: AutomationCase = {
      id: `case-${Date.now()}`, title: ex.title, description: ex.description,
      sourceRunId: parentRun?.id || '', sourceExceptionId: exId,
      severity: ex.severity, category: ex.category as CaseCategory, owner: engagement.owner,
      dueDate: futureDate(14), status: 'OPEN', priority: ex.severity,
      evidenceRefs: [], comments: '', createdAt: new Date().toISOString().slice(0, 10), updatedAt: new Date().toISOString().slice(0, 10),
      ...
      {caseCandidates.length > 0 && (
        <div className="rounded-lg border border-amber-200/50 bg-amber-50/20 p-4 space-y-2">
          ...
          <div className="space-y-1.5">{caseCandidates.map(ex => (
            <div key={ex.id} className="flex items-center justify-between rounded-lg border border-border-light bg-white px-3 py-2">
          ...
          <table className="w-full text-[11px]">
            ...
            <tbody>{casesState.cases.map(c => (
              <tr key={c.id} className="border-b border-border-light/50">
                ...
                <td className="px-3 py-2 text-center text-gray-500">{c.evidenceRefs.length || '—'}</td>
  ...
  return (
    <div className="rounded-lg border-2 border-primary/20 bg-primary/5 p-4 space-y-3">
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <button onClick={() => { if (!title.trim()) return; onSave({ id: `case-${Date.now()}`, title: title.trim(), description: desc.trim(), sourceRunId: '', sourceExceptionId: '', severity: priority, category, owner, dueDate, status: 'OPEN', priority, evidenceRefs: [], comments: '', createdAt: new Date().toISOString().slice(0, 10), updatedAt: new Date().toISOString().slice(0, 10), history: [{ id: `ch-${Date.now()}`, action: 'CREATED', actor: engagement.owner, timestamp: now(), comments: 'Manual case.' }] }); }}
...
function CaseDetailPanel({ caseItem, onUpdate, onTransition, onAddEvidence, onClose }: {
  ...
  return (
    <div className="rounded-xl border-2 border-primary/20 bg-white p-4 space-y-3 shadow-lg">
      ...
      {caseItem.sourceExceptionId && <div className="text-[10px] text-gray-500">Source: Exception {caseItem.sourceExceptionId.slice(-8)}</div>}
      {/* History */}
      ...
      {/* Actions */}
      <div className="flex items-center gap-2">
        <button onClick={() => { onUpdate({ title: draft.title, description: draft.description, owner: draft.owner, priority: draft.priority, dueDate: draft.dueDate, comments: draft.comments }); onClose(); }} className="px-3 py-1.5 rounded-lg bg-primary hover:bg-primary/90 text-white text-[11px] font-semibold cursor-pointer transition-colors">Save</button>

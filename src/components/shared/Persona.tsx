...
export const Persona: FC<PersonaProps> = memo(
  ({ variant = 'halo', state = 'idle', onLoad, onLoadError, onReady, onPause, onPlay, onStop, className }) => {
    ...
    const stableCallbacks = useMemo(
      () => ({
        onLoad: ((loadedRive: Parameters<NonNullable<RiveParameters['onLoad']>>[0]) => callbacksRef.current.onLoad?.(loadedRive)) as RiveParameters['onLoad'],
        onLoadError: ((err: Parameters<NonNullable<RiveParameters['onLoadError']>>[0]) => callbacksRef.current.onLoadError?.(err)) as RiveParameters['onLoadError'],
        onReady: () => callbacksRef.current.onReady?.(),
        onPause: ((event: Parameters<NonNullable<RiveParameters['onPause']>>[0]) => callbacksRef.current.onPause?.(event)) as RiveParameters['onPause'],
        onPlay: ((event: Parameters<NonNullable<RiveParameters['onPlay']>>[0]) => callbacksRef.current.onPlay?.(event)) as RiveParameters['onPlay'],
        onStop: ((event: Parameters<NonNullable<RiveParameters['onStop']>>[0]) => callbacksRef.current.onStop?.(event)) as RiveParameters['onStop'],
      }),
      [],
    );
    ...
    return (
      <Component rive={rive} source={source}>
        <RiveComponent className={`w-16 h-16 shrink-0 ${className || ''}`} />
      </Component>
    );
  },
);
